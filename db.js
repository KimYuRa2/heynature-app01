...
function insertNotice(title,content,callback){
    connection.query(`insert into notice (title,create_time, update_time,content) values ("${title}",now(),now(),"${content}")`,(err,result)=>{
        if(err) throw err;
        callback();
    })
...
function updateNoticeById(id, title, content, callback){
    connection.query(`update notice set title='${title}',update_time=now(), content='${content}' where id=${id}`, (err, result)=>{
        if(err) throw err;
        callback();
    })
}
